...
std::string writeFinalTable(int n, int m, std::vector<std::vector<double>> v, double a, double b, double c, double d, int S, int S_2, double eps, double eps_max, double eps_max_2, double error_max, double r_norm, double w, double accuracy) {
    ...
    outfile << "\nТаблица № " << 7 << "\n\n" << std::endl;
    std::string returned_string = std::string("Параметр t: ") + doubleToString(w) + "\n\nЧисло шагов: " + doubleToString(S) +  "\n\nТочность метода: " +
...
double t_optimal(double k, double h, const int n, const int m) {
    ...
    double lmin = -4 / h2 * pow(sin(M_PI_2 * n), 2) - 4 / k2 * pow(sin(M_PI_2 * m), 2);
    double lmax = -4 / h2 * pow(cos(M_PI_2 * n), 2) - 4 / k2 * pow(cos(M_PI_2 * m), 2);
    double mu = lmax / lmin;
...
inline bool is_border(int i, int j, int n, int m) {
    if (i == 0 && (j >= 0 && j <= 3 * m / 4)) return true;  // 1
    if (i == n) return true;  // 2
    if (j == 0) return true;  // 3
    if (j == m && i >= n / 4) return true;  // 4
    if (j == m / 4 && i >= n / 2 && i <= 3 * n / 4) return true;  // 5
    if (j == 3 * m / 4 && i >= n / 2 && i <= 3 * n / 4) return true;  // 6
    if (i == n / 2 && j >= m / 4 && j <= 3 * m / 4) return true;  // 7
    if (i == 3 * n / 4 && j >= m / 4 && j <= 3 * m / 4) return true;  // 8
    if (i >= 1 && i <= n / 4 && j == 3 * m / 4) return true;  // 9
    if (i == n / 4 && j >= 3 * m / 4 && j <= m) return true;  // 10
    return false;
}
...
TResults solve(std::vector<std::vector<double>>& v, Function func, const int n, const int m, double a, double b, double c, double d, int Nmax, int& S, double& eps, double& eps_max, double& error_max) {
    ...
    bool flag = S>=Nmax; // условие остановки
    ...
    for (i = n / 2; i <= 3 * n / 4; i++) {
        v[i][m / 4] = u_test(a + i * (b - a) / n, c + (d - c) / 4);
    }

    for (i = n / 2; i <= 3 * n / 4; i++) {
        v[i][3 * m / 4] = u_test(a + i * (b - a) / n, c + 3 * (d - c) / 4);
    }

    for (j = m / 4; j <= 3 * m / 4; j++) {
        v[n / 2][j] = u_test(a + (b - a) / 2, c + (d - c) / m * j);
    }

    for (j = m / 4; j <= 3 * m / 4; j++) {
        v[3 * n / 4][j] = u_test(a + 3 * (b - a) / 4, c + (d - c) / m * j);
    }
    ...
    for (j = 3 * m / 4; j <= m; j++) {
        v[n / 4][j] = u_test(a + (b - a) / 4, c + (d - c) / m * j);
    }
